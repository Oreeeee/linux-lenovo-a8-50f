...
static void captouch_early_suspend(struct early_suspend *h);
static void captouch_late_resume(struct early_suspend *h);
...
static int captouch_probe(struct platform_device *pdev) 
{
    ...
    atomic_set(&(captouch_context_obj->early_suspend), 0);
	captouch_context_obj->early_drv.level    = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 1,
	captouch_context_obj->early_drv.suspend  = captouch_early_suspend,
	captouch_context_obj->early_drv.resume   = captouch_late_resume,    
	register_early_suspend(&captouch_context_obj->early_drv);
#endif
	...
	CAPTOUCH_LOG("----captouch_probe fail !!!\n");
	return err;
}
...
static void captouch_early_suspend(struct early_suspend *h) 
{
   atomic_set(&(captouch_context_obj->early_suspend), 1);
   CAPTOUCH_LOG(" captouch_early_suspend ok------->hwm_obj->early_suspend=%d \n",atomic_read(&(captouch_context_obj->early_suspend)));
   return ;
}
/*----------------------------------------------------------------------------*/
static void captouch_late_resume(struct early_suspend *h)
{
   atomic_set(&(captouch_context_obj->early_suspend), 0);
   CAPTOUCH_LOG(" captouch_late_resume ok------->hwm_obj->early_suspend=%d \n",atomic_read(&(captouch_context_obj->early_suspend)));
   return ;
}

static int captouch_suspend(struct platform_device *dev, pm_message_t state) 
{
	return 0;
}
/*----------------------------------------------------------------------------*/
static int captouch_resume(struct platform_device *dev)
{
	return 0;
}
...
static const struct of_device_id captouch_of_match[] = {
	{ .compatible = "mediatek,m_captouch_pl", },
	{},
};
#endif

static struct platform_driver captouch_driver =
{
	.probe      = captouch_probe,
	.remove     = captouch_remove,    
	.suspend    = captouch_suspend,
	.resume     = captouch_resume,
	.driver     = 
	{
		.name = CAPTOUCH_PL_DEV_NAME,
        #ifdef CONFIG_OF
...
static int __init captouch_init(void) 
{
	...
	if(platform_driver_register(&captouch_driver))
	{
...
{
	platform_driver_unregister(&captouch_driver);    
